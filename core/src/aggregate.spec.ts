...
test("aggregate - After Push > Push > Push > Pop > Replace", () => {
  const t = nowTime();

  let pushedEvent0: PushedEvent;
  let pushedEvent1: PushedEvent;
  let pushedEvent2: PushedEvent;
  let poppedEvent1: PoppedEvent;
  let replacedEvent1: ReplacedEvent;
  ...
  const output = aggregate(events, t + 500);
