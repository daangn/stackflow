...
> ### Pageview event가 두 번씩 발생하는 이슈가 있어요.

GA4 설정에서 "Page changes based on browser history events"를 해제하세요. (**_Web Stream>Enhanced Measurement>Pageviews>Advanced_**)

이 플러그인은 스택플로우의 "[Effect Hook](https://stackflow.so/guided-tour/write-plugin#%EC%9D%B4%ED%8E%99%ED%8A%B8-%ED%9B%85)"을 사용하여 페이지뷰 이벤트를 수동으로 트리거합니다. 다시 트리거할 필요가 없습니다.
