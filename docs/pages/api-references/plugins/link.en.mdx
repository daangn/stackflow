...
It can be used only when [`@stackflow/plugin-history-sync`](/api-references/plugins/plugin-history-sync) is set.
...
| animate           | `boolean` (optional)                                | Indicates whether to animate the transition when navigating. If not provided, the default transition is used.    |
