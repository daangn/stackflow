...
<Callout emoji="💡">
  이제 플러그인에서 필요한 추가적인 설정이 있을때, 플러그인 함수의 파라미터 뿐 아니라 `@stackflow/config`를 확장하여 플러그인 구동에 필요한 정보를 받을 수도 있습니다.
</Callout>

### `decorate` API

다음과 같이 Config 타입을 확장해서 `config`에 접근할 수 있는 다양한 장소에 활용할 수 있습니다.
