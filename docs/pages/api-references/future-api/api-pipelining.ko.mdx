...
위와 같이 엔트리 파일에서 리액트 앱 초기화와 API 요청을 동시에 수행해 초기 렌더링까지 걸리는 시간을 단축할 수 있습니다. Stackflow Future API를 활용하면 API 파이프라이닝을 깔끔한 방법으로 구현할 수 있습니다.

```typescript showLineNumbers filename="entry.ts" copy
...
```typescript showLineNumbers filename="renderApp.ts" copy
