...
만약 특정 시점에 다음 화면의 API를 미리 가져오고 싶다면, `plugin-preload`에서 제공하는 `usePreloader()` 훅을 통해 `preload` 함수를 사용할 수 있어요.

`createPreloader()` 함수를 통해 안전하게 타입된 `usePreloader()` 훅을 만들어요.
...
// 등록된 액티비티의 전체 타입을 Generic으로 전달해, 안전하게 타입된 usePreloader 훅을 사용해요
...
`createLink()` 함수를 통해 안전하게 타입된 Link 컴포넌트를 만들어요.
...
<Callout emoji="⚡️">
  `<Link />` 컴포넌트는 기본적으로 사용자의 뷰포트 안에 해당 컴포넌트가
  노출되었을때 미리 데이터를 가져와요. 따라서, 사용자에게 로딩이 없는 탐험
  경험을 제공할 수 있어요.
</Callout>
