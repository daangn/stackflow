...
### Stackflow 초기화하기
프로젝트 내에 JavaScript(TypeScript) 파일을 하나 생성하고, `stackflow()` 함수에 config와 액티비티 컴포넌트를 전달해 `<Stack />`과 `useFlow()` 함수를 생성해요.

다른 컴포넌트에서 `<Stack />`과 `useFlow()`를 활용할 수 있도록 `export ...` 해줘요.
...
```ts showLineNumbers filename="stackflow.ts" copy {9-14}
import { stackflow } from "@stackflow/react";
