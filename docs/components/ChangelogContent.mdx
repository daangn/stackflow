...
Expand the supported `@stackflow/core` peer dependency range to include both v2 and v3. [`a201469`](https://github.com/daangn/stackflow/commit/a2014697b7bc8488d354cea93d4bbfb75a4c4a6b)
...
Preserve snapshot navigation history during initialization instead of replacing it with events derived from the current URL. `@stackflow/core` v3 is now required so the plugin can distinguish snapshot loads from fresh stack creation. [`f7c3a2b`](https://github.com/daangn/stackflow/commit/f7c3a2ba5c0adbf96d34fb1b02cae577c28c325d)
...
Support `preventDefault`: the browser history now follows the committed stack through a single reconciler, so `preventDefault`-based plugins (e.g. `@stackflow/plugin-blocker`) work with browser back/forward and programmatic navigation without history/stack desync. [`78ea192`](https://github.com/daangn/stackflow/commit/78ea192e8b0bdbbc02e72384656b20977137fb04)
