...
import type { BeforePushType, KarrotframePlugin, PluginType } from "@karrotframe/plugins";
...
test('인터페이스에 맞게 플러그인을 선언하면 정상적으로 렌더링한다.', () => {
    // given
    const SimplePluginContext = createContext<{ data: any; setData: (data: any) => void; }>(null as any)
...
test('플러그인의 hook 에서 얻은 정보로 컴포넌트를 업데이트한다.', async () => {
    ...
    const useSimplePlugin = (): PluginType & { displayCurrentPath: () => string; }  => {
        const context = useContext(SimplePluginContext);
        return useMemo(() => {
            return {
                lifeCycleHooks: {
    ...
    const TargetPage: React.FC = () => {
        ...
        return (<div onClick={(() => {
            push('/');
        })}>{displayCurrentPath() || "no path"}</div>)}
...
test('플러그인을 사용해서 미들웨어의 마지막 단에 가공한 url 로 이동한다.', async () => {
    // given
    const modifyUrlMiddleware = async (ctx: BeforePushType, next: (ctx?: BeforePushType) => Promise<BeforePushType | void>): Promise<BeforePushType | void>   => {
        if (ctx.to === '/false-url') {
            await next({
                ...ctx,
                to: '/correct-url',
    ...
    const loggerMiddleware = async (ctx: BeforePushType, next: (ctx?: BeforePushType) => Promise<BeforePushType | void>): Promise<BeforePushType | void>  => {
        console.log('logger : ', ctx.to);
        next(ctx);
    }
    const redirectMiddleware = async (ctx: BeforePushType): Promise<BeforePushType | void>  => {
        ctx.options.push?.(ctx.to);
    }

    const useTestMiddlewarePlugin = (): PluginType => ({
        lifeCycleHooks: {
            beforePush: composeMiddlewares<BeforePushType>([modifyUrlMiddleware, loggerMiddleware, redirectMiddleware])
        }
    })
...
test('플러그인을 2개 이상 사용할 때 플러그인을 배열에 선언한 순서에 따라 플러그인이 작동한다.', async () => {
    // given
    const redirectFirst = async (ctx: BeforePushType): Promise<BeforePushType | void>   => {
        if (ctx.to === '/move') {
            ctx.options.push?.('/first-url')
        }
    }
    const redirectSecond = async (ctx: BeforePushType): Promise<BeforePushType | void>   => {
        if (ctx.to === '/first-url') {
